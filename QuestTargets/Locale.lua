...
local phrases = {
    ...
    dbInstallText = {"Install QuestieDB Classic (Vanilla) for known quest targets. Select the link and press Ctrl+C to copy it:", "Installiere QuestieDB Classic (Vanilla) für bekannte Questziele. Link markieren und mit Strg+C kopieren:", "Instala QuestieDB Classic (Vanilla) para conocer los objetivos. Selecciona el enlace y cópialo con Ctrl+C:", "Installez QuestieDB Classic (Vanilla) pour les cibles connues. Sélectionnez le lien et copiez-le avec Ctrl+C :", "Bilinen görev hedefleri için QuestieDB Classic (Vanilla) kurun. Bağlantıyı seçip Ctrl+C ile kopyalayın:", "安装 QuestieDB Classic (Vanilla) 以获取已知任务目标。选中链接并按 Ctrl+C 复制："},
